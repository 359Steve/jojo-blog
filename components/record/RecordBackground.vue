...
const { className = '', showRadialGradient = true } = defineProps<{
	className?: string;
...
<template>
	<div v-bind="$attrs" :class="['transition-bg bg-zinc-50 dark:bg-zinc-900', className]">
		<div class="absolute inset-0 overflow-hidden">
			<div
				:class="[
					...
					showRadialGradient ? '[mask-image:radial-gradient(ellipse_at_100%_0%,black_10%,var(--transparent)_90%)]' : '',
				]"
			></div>
		</div>
		<slot></slot>

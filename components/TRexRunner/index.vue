...
import DinoRun1 from '~/assets/image/dino/dino-run-0.png';
import DinoRun2 from '~/assets/image/dino/dino-run-1.png';
import DinoStatic from '~/assets/image/dino/dino-stationary.png';
import DinoLose from '~/assets/image/dino/dino-lose.png';
import Ground from '~/assets/image/dino/ground.png';
import Cactus from '~/assets/image/dino/cactus.png';
...
// 恐龙失败图片
const dinoLoseImg = computed(() => createImage(DinoLose));
// 道路图片
const groundImg = computed(() => createImage(Ground));
// 仙人掌图片
const cactusImg = computed(() => createImage(Cactus));
...
// 更新仙人掌
const updateCactus = (delta: number) => {
	...
	cactuses.value.forEach((item) => (item.x -= delta * 0.3 * gameState.speedScale));
	cactuses.value = cactuses.value.filter((item) => item.x + item.width > 0);
};
...
// 加载画面
const renderGame = (playing: boolean) => {
...
<template>
	<div ref="DinoBox" class="relative mt-12 h-[400px] w-full">
		<div class="relative flex h-1/2 w-full items-center justify-center p-4">
			<div
				class="absolute right-4 top-4 z-10 rounded bg-white/70 px-3 py-1 text-sm font-medium text-gray-800 shadow-md backdrop-blur-md dark:bg-gray-800/70 dark:text-gray-200">

...
const { data: discounts, status } = await useAsyncData('cart-discount', async () => {
	const [coupons, offers] = await Promise.all([useFetch('/cart/coupons'), useFetch('/cart/offers')]);
...
const simulateRequest = async () => {
	loading.start(); // 手动开启
	try {
		await new Promise((resolve) => setTimeout(resolve, 2000)); // 模拟耗时请求
		loading.finish(); // 成功后关闭
	} catch (err) {
		loading.error(); // 失败时显示错误状态
	}
};
```

...
const play = (): void => {
	...
	pathRefs.value!.forEach((path, i) => {
		setTimeout(() => {
			path.style.animation = `grow 5s ease forwards`;
		}, times.value[i] * 1000);
	});
};
	...
	setTimeout(
		() => {
			...
			reversed.forEach((path, i) => {
				setTimeout(() => {
					path.style.animation = `shrink 1s ease forwards`;
				}, i * 500);
			});
		},
...
<template>
	<svg class="h-full stroke-black dark:stroke-white" viewBox="0 0 174.17534 103.05544" @click="navigateTo('/')">
		<g id="svgGroup">
			<path id="path2" :ref="ref => (pathRefs[0] = ref as SVGPathElement)" style="
					fill: none;
					fill-opacity: 0;
			...
			<path id="path3" :ref="ref => (pathRefs[1] = ref as SVGPathElement)" style="
					fill: none;
					fill-opacity: 0;
					stroke-width: 3.77953;
			...
			<path id="path4" :ref="ref => (pathRefs[2] = ref as SVGPathElement)" style="
					fill: none;
					fill-opacity: 0;
					stroke-width: 3.77953;
					stroke-linecap: round;
			...
			<path id="path5" :ref="ref => (pathRefs[3] = ref as SVGPathElement)" style="
					fill: none;
					fill-opacity: 0;
					stroke-width: 3.77953;
					stroke-linecap: square;
			...
			<path id="path6" :ref="ref => (pathRefs[4] = ref as SVGPathElement)" style="
					fill: none;
					fill-opacity: 0;
					stroke-width: 3.77953;
					stroke-linecap: butt;
